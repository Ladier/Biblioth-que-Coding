<?php
    ...
    $queryInfo = $pdo->prepare("SELECT books.titre, books.img_src, books.resume, books.date_edition, authors.nom, genres.name FROM books INNER JOIN authors ON books.author_id = authors.id INNER JOIN composition ON books.id = composition.livre_id INNER JOIN genres ON composition.genre_id = genres.id WHERE genres.id = :id");
